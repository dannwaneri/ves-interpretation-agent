...
3. **Arithmetic that doesn't reconcile, printed in the paper itself, twice.** The Etche paper's own Table 1 for the Egwi station prints layer 3's cumulative depth as 6.2935 m and layer 4's thickness as 37.957 m, which sums to 44.2505 m, but the same table prints layer 4's cumulative depth as 37.25 m. That's the paper's own arithmetic, not a transcription error on this project's side: the raw extracted PDF text (`eval/paper-text/etche.txt`) confirms it, and the paper's own prose describing Figure 2 *independently repeats both* the ~37.95 m thickness and the 37.25 m depth. So this isn't a single isolated typo in one table cell; the paper states both numbers consistently in two places, and they still don't reconcile with each other. Which one is the actual error, if either, can't be determined without the raw field data, so the agent reports the inconsistency and both possible readings rather than picking a winner (`agent/depthArithmetic.js`). Five other stations have the same kind of inconsistency (Ulakwo, Okehi, Akpoku, Ndashi, Umuokom); see [docs/conflicts-checklist.md](docs/conflicts-checklist.md).
...
3. Runs three code-computed checks against the GROQ data before the model ever sees it as a fact to narrate, not derive:
   ...
   - **Depth arithmetic** (`agent/depthArithmetic.js`): checks each layer's printed cumulative depth against prior depth + thickness, tolerant of ordinary rounding but not a multi-meter inconsistency. Reports both possible readings (which number would need to change for the layer to reconcile) rather than asserting which printed value is the error, since that can't be determined from the paper alone; see [Limitations](#limitations).
...
- **The agent's first answer to the Egwi depth-arithmetic question asserted a winner it shouldn't have.** It said the printed depth "is wrong by about 7 meters," which implies the printed thickness is trustworthy and the depth isn't. Checking the PDF further showed the paper's own prose (describing Figure 2, separate from Table 1) independently repeats both numbers, so there's no basis for picking one as the error. Fixed by wiring a depth-arithmetic check into the online and `--public` pipelines (it was previously only in `--offline`) and rewriting the prompt to require both possible readings be stated, not a winner. Even after two rounds of tightening, wording across repeated runs isn't perfectly symmetric (an LLM's prose varies run to run), though no run since the fix has asserted a specific number is the error.
